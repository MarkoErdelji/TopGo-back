...
public interface RideRepository extends JpaRepository<Ride,Integer> {
    ...
    @Query("SELECT r FROM Ride r LEFT JOIN r.passenger p WHERE (r.driver.id = :userId OR p.id = :userId) AND (r.start BETWEEN :startDate AND :endDate)")
    ...
    @Query("SELECT  r FROM Ride r LEFT JOIN r.passenger p WHERE  p.id = :userId AND (r.start BETWEEN :startDate AND :endDate)")
    ...
    @Query("SELECT  r FROM Ride r LEFT JOIN r.driver p WHERE  p.id = :userId AND (r.start BETWEEN :startDate AND :endDate)")
    ...
    @Query("SELECT  r FROM Ride r  LEFT JOIN r.passenger p WHERE (p.id = :passengerId) AND (r.status = 3)")
    List<Ride> findRidesByPassengeridAndIsActive(@Param("passengerId") int passengerId);
    @Query("SELECT  r FROM Ride r  LEFT JOIN r.passenger p WHERE (p.id = :passengerId) AND (r.status = 1)")
    List<Ride> findRidesByPassengeridAndIsAccepted(@Param("passengerId") int passengerId);

    @Query("SELECT  r FROM Ride r  LEFT JOIN r.passenger p WHERE (p.id = :passengerId) AND (r.status = 0)")
